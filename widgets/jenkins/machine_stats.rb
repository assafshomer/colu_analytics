...
init_time = Time.now
mainnet_reply = JENKINS.api_get_request(mainnet_request)
mainnet_status = mainnet_reply['result']
p "JENKINS API replied within [#{time_diff(init_time)}]"

p "Getting Testnet Machine Status from Jenkins"
init_time = Time.now
testnet_reply = JENKINS.api_get_request(testnet_request)
testnet_status = testnet_reply['result']
p "JENKINS API replied within [#{time_diff(init_time)}]"
